                ...
                <div class="x_content">
                    ...
                    <div class="box-body">
                        ...
                        <div class="form-group row">
                            ...
                            <form id="mergeForm" class="mergeForm" method="post" action="{{ url('merge-onboarder-traders/post') }}">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label class="control-label" for="dealer">Onboarders</label>
                                        <select name="onboarders" class="filter form-control" id="onboarders">
                                            <option>Select</option>
                                            @foreach($onboarders as $onboarder)
                                            <option value="{{ $onboarder->id }}">{{ $onboarder->name }}</option>
                                            @endforeach
                                        </select>
                                        <div id="message"></div>
                                    </div>
...
<script type='text/javascript'>
    $(document).ready(function () {
        ...
        var table = $('#traders-table').DataTable({
            ...
            ordering: true,
            ajax: {
                url: '{!! route('merge-onboarder-trader-data') !!}'
            },
            columns: [{
                    data: 'rownum',
                    name: 'rownum',
        ...
        frm.submit(function (e) {
            ...
            $.ajax({
                ...
                url: frm.attr('action'),
                // data: $('.merge_check:checked').serialize() + '&onboarders=' + $('#onboarders').val(),
                data: {merge:selectedCheck, onboarders:$('#onboarders').val()},
