...
class Controller extends BaseController {
    ...
    public function sendAuctionOwnerNegotiatedEndPush($auctionId){
        ...
        //$msg = 'You won the auction of '.$auction->title.' for USD '.(int) $lastBid;

        $msg = $auction->title . ', Thank you for your offer of USD ' . (int) $lastBid . '. Please wait whilst we review your offer.';

        //Thank you for your offer of USDX. Please wait whilst we review your offer.
        //echo $msg; exit;
    ...
    public function sendAuctionCompleteMail($auctionId) {
        $auction = Auction::find($auctionId);
        $DealerUser = DealerUser::where('id', $auction->dealer_id)->first();
        $data = [
            'subject' => 'Auction completed',
            'email' => $DealerUser->email,
            'content' => 'Auction for ' . $auction->title . ' is complete. We will notify you once its cashed.'
          ];
        try {
    ...
    public function sendAuctionEndPush($auctionId) {
        ...
        $msg = $auction->title . ', Thank you for your offer of USD ' . (int) $lastBid . '. Please wait while we review your offer.';
        ...
        //$msg = 'Auction for '.$auction->title.' is completed and gone for USD '.(int) $lastBid;
    ...
    public function sendAuctionEndPushToDealer($auctionId) {
        ...
        $msg = $auction->title . ', Thank you for your offer of USD ' . (int) $lastBid . '. Dealer accepted your offer we will contact you soon.';
    ...
    public function sendSms($auctionId, $type) {
        return true;
         try {
              ...
              if ($type == 1) {
                   $msg = 'Congratulations. Your bid of USD ' . $lastBid->price . ' has been accepted for ' . $auction->title . '. Please await whilst your car clears Quality Control.';
              } elseif ($type == 2) {
                   $msg = $auction->title . ' has cleared Quality Control. Please make a payment of USD ' . $lastBid->price . ' within 24 hours quoting ' . $auction->title;
              } elseif ($type == 3) {
      ...
      return;
         try {
              ...
              if(!empty($trader->mobile)){
                  ...
                  $msg = $auction->title . ', Thank you for your offer of USD ' . (int) $lastBid->price . '. Please wait whilst we review your offer.';
                  // $msg = 'Congratulations. Your bid of USD ' . $lastBid->price . ' has been accepted for ' . $auction->title . '. Please await whilst your car clears Quality Control.';
    ...
    public function sendAuctionOwnerNegotiatedPush($auctionId) {
        ...
        $msg = $auction->title . ' request you to bid for USD ' . (int) $auction->final_req_amount;
        //echo $msg; exit;
        $devices['iosDevices'] = array();
        $devices['androidDevices'] = array();
    ...
    public function sendAuctionNegotiatedPush($auctionId) {
        ...
        $msg = $auction->title . ' bid negotiated and started again with USD ' . (int) $lastBid;
        //echo $msg; exit;
    ...
    public function sendBidPush($auctionId, $bidPrice = '') {
        ...
        $auction = Auction::find($auctionId);
        //$msg = $auction->title.' bid price updated to USD '. (int) $bidPrice;
        ...
        $msg = $auction->title . ', You have bid USD ' . (int) $bidPrice;
